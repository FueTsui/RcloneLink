...
class AddConfigDialog(QDialog):
    ...
    def config_finished(self, success, message):
        ...
        if success:
            ...
            QMessageBox.critical(self, "失败", f"配置失败: {message}")
...
class ConfigTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.webdav_url_edit = QLineEdit()
        self.webdav_url_edit.setFixedHeight(30)
        self.webdav_url_edit.setPlaceholderText("https://192.168.1.1:443/dav/")
...
class MountTab(QWidget):
    ...
    def save_mount_list(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"无法保存挂载列表: {e}")
    ...
    def load_mount_list(self, file_path=None, auto_start=False):
        ...
        try:
            ...
            if is_user_action:
                QMessageBox.critical(self, "错误", f"无法加载挂载列表: {e}")
            return False
...
class SettingsTab(QWidget):
    ...
    def toggle_autostart(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"无法更新开机启动设置: {e}\n请尝试以管理员身份运行此程序。" )
...
def show_winfsp_install_prompt():
    ...
    msg_box.setInformativeText("您可以从当前目录下的安装包安装，或访问官网下载。")

    install_button = msg_box.addButton("从本地安装", QMessageBox.ActionRole)
    ...
    if msg_box.clickedButton() == install_button:
        ...
        if os.path.exists(installer_path):
            try:
                ...
                QMessageBox.information(None, "提示", "WinFsp 安装程序已启动。\n请在安装完成后重新启动 RcloneLink。")
            except Exception as e:
                QMessageBox.critical(None, "错误", f"无法启动 WinFsp 安装程序: {e}\n请尝试手动右键以管理员身份运行 winfsp-2.1.25156.msi。")
        else:
            QMessageBox.critical(None, "错误", f"未找到安装文件: {installer_path}")
        return False  # Exit app
    elif msg_box.clickedButton() == website_button:
        webbrowser.open("https://winfsp.dev/rel/")
        QMessageBox.information(None, "提示", "请在下载并安装 WinFsp 后重新启动 RcloneLink。")
        return False  # Exit app
