...
from PySide6.QtWidgets import (
    ...
    QDialog, QSystemTrayIcon, QMenu, QHeaderView
)
...
def remove_existing_remote(config_content: str, remote_name: str) -> str:
    """从配置中移除已存在的远程"""
    ...
    return '\n'.join(result_lines)
from mounts import get_column_headers, populate_mount_table_row, read_mount_options_from_row, start_mount_all, get_default_mount_options, get_next_available_drive
    ...
    def create_config_file(self, config: Dict) -> bool:
        ...
        try:
            ...
            if f"[{config['name']}]" in existing_config:
                ...
                existing_config = remove_existing_remote(existing_config, config['name'])
...
class ConfigTab(QWidget):
    ...
    def remove_remote(self):
        ...
        if ok and remote_to_delete:
            ...
            new_config_content = remove_existing_remote(config_content, remote_to_delete)
...
class MountTab(QWidget):
    ...
    def stop_mount_for_remote(self, remote_name):
        """停止特定远程的挂载(异步)"""
        if remote_name in self.active_mounts:
            # 使用与 stop_mount 中相同的 UnmountWorker 来确保异步执行
            ...
            mounts_to_stop = [{
                'worker': worker, 'thread': thread, 'mount_point': mount_point,
                'network_mode': network_mode, 'remote_name': remote_name
            }]

            # 由于 UnmountWorker 定义在 stop_mount 方法内部，
            # 我们需要一个通用的方法来处理异步卸载。
            # 这里我们创建一个临时的线程来处理这个单一任务。
            unmount_thread = QThread(self)
            
            # 假设 UnmountWorker 可以在这里被访问到，或者需要重新定义
            # 为了简单起见，我们直接调用 worker.stop，但在一个新线程中
            class SingleUnmountWorker(QObject):
                finished = Signal()
                def __init__(self, stop_func, quit_func, wait_func):
                    super().__init__()
                ...
                def run(self):
                    self.stop_func()
                    self.quit_func()
                    self.wait_func(1000) # 等待1秒
                    self.finished.emit()

            stop_worker = SingleUnmountWorker(
                lambda: worker.stop(mount_point, network_mode),
                thread.quit,
                thread.wait
            )
            stop_worker.moveToThread(unmount_thread)
            unmount_thread.started.connect(stop_worker.run)
            stop_worker.finished.connect(unmount_thread.quit)
            # 清理
            stop_worker.finished.connect(stop_worker.deleteLater)
            unmount_thread.finished.connect(unmount_thread.deleteLater)
            
            self.active_mounts.pop(remote_name, None)
            self.mount_output.append(f"开始在后台停止挂载 '{remote_name}'...")
            unmount_thread.start()
            # 保留对线程的引用以防被垃圾回收
            if not hasattr(self, 'single_unmount_threads'):
                self.single_unmount_threads = []
            self.single_unmount_threads.append(unmount_thread)
            unmount_thread.finished.connect(lambda t=unmount_thread: self.single_unmount_threads.remove(t))
    ...
    def stop_mount(self):
        """停止挂载（异步）"""
        if not self.is_mounted or not self.active_mounts:
            return

        class UnmountWorker(QObject):
            """执行卸载操作的工作线程"""
            finished = Signal()
            update_output = Signal(str)
            unmount_complete = Signal(str)

            def __init__(self, mounts_to_stop):
                super().__init__()
                self.mounts_to_stop = mounts_to_stop

            def run(self):
                for mount_info in self.mounts_to_stop:
                    worker = mount_info['worker']
                    thread = mount_info['thread']
                    mount_point = mount_info['mount_point']
                    network_mode = mount_info['network_mode']
                    remote_name = mount_info['remote_name']
                    ...
                    if thread.isRunning():
                        thread.terminate()
                        thread.wait()
                    self.unmount_complete.emit(remote_name)
                self.finished.emit()

        self.mount_output.append("正在停止所有挂载...")
        self.mount_btn.setEnabled(False)
        self.mount_btn.setText("正在停止...")

        mounts_to_stop = []
        for remote_name, (worker, thread) in self.active_mounts.items():
            mount_point = ""
            network_mode = False
            if remote_name in self.remote_rows:
                row = self.remote_rows[remote_name]
            ...
            mounts_to_stop.append({
                'worker': worker, 'thread': thread, 'mount_point': mount_point,
                'network_mode': network_mode, 'remote_name': remote_name
            })

        self.unmount_thread = QThread(self)
        self.unmount_worker = UnmountWorker(mounts_to_stop)
        self.unmount_worker.moveToThread(self.unmount_thread)

        def on_unmount_complete(remote_name):
            self.active_mounts.pop(remote_name, None)
            self.update_mount_status(remote_name, "已停止")
            self.mount_output.append(f"已停止挂载远程 '{remote_name}'")

        def on_stop_mount_finished():
            self.is_mounted = False
            self.mount_btn.setText("开始挂载")
            self.mount_btn.setEnabled(True)
            self.mount_output.append("所有挂载已停止")
            self.unmount_thread.quit()
            self.unmount_thread.wait()

        self.unmount_worker.unmount_complete.connect(on_unmount_complete)
        self.unmount_worker.finished.connect(on_stop_mount_finished)
        self.unmount_thread.started.connect(self.unmount_worker.run)
